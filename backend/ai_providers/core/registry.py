...
class CircuitBreaker:
    ...
    def is_available(self) -> bool:
        ...
        if self._state.state == "open":
            if self._state.next_attempt_time and datetime.now(UTC) >= self._state.next_attempt_time:
    ...
    def record_failure(self) -> None:
        ...
        if self._state.state == "half_open":
            ...
            self._state.next_attempt_time = datetime.now(UTC) + timedelta(seconds=self.recovery_timeout)
            self._half_open_calls = 0
